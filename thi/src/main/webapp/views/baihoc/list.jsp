    ...
    <span>
        ...
        <div class="col-lg-12">
            <table id="tableCustomer" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                <c:forEach var="facility" items="${listCustomer}">
                    <tr>
                        <td><c:out value="${facility.maKhachHang}"/></td>
                        <td>
                            <c:forEach var="customerType" items="${listCustomerType}">
                                <c:if test = "${facility.maLoaiKhach==customerType.maLoaiKhach}"> ${customerType.tenLoaiKhach}</c:if>
                            </c:forEach>
                                <%--                            <c:out value="${customer.maLoaiKhach}"/>--%>
                        </td>
                        <td><c:out value="${facility.hoTen}"/></td>
                        <td><c:out value="${facility.ngaySinh}"/></td>
                        <td>
                            <c:if test = "${facility.gioiTinh==1}">
                                <span value ="1" >Nam</span>
                            </c:if>
                            <c:if test = "${facility.gioiTinh==0}">
                                <span value ="0" >Nữ</span>
                            </c:if>
                        </td>
                        <td><c:out value="${facility.soCMND}"/></td>
                        <td><c:out value="${facility.soDienThoai}"/></td>
                        <td><c:out value="${facility.email}"/></td>
                        <td><c:out value="${facility.diaChi}"/></td>

                        <td>
                            <a href="/customer?action=edit&ma_khach_hang=${facility.maKhachHang}">
                        ...
                        <td>
                            ...
                            <button type="button" class="btn btn-primary bg-danger" data-bs-toggle="modal"
                                    ...
                                    onclick="setIdToFormDelete('${facility.maKhachHang}')">
                                Delete
                            </button>
